...
def BiRNNAndLogistic(xTem ,xStatic , weights, biases,step,n_hidden):
    ...
    out = tf.matmul(outputs[-1], weights[0]) + biases[0]+ tf.matmul(xStatic,weights[1])
...
pred = BiRNNAndLogistic(xTem, xSta, weights, biases,time_step,n_hidden)
